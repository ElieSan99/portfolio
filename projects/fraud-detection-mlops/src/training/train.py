...
def train_model(X: pd.DataFrame, y: pd.Series) -> None:
    ...
    # Mise à l'échelle des données pour assurer la convergence
    ...
    with mlflow.start_run():
        ...
        # Signature du modèle pour éviter les warnings
        signature = infer_signature(X_train_scaled, y_pred)
        mlflow.sklearn.log_model(model, artifact_path="model", signature=signature)

        # Sauvegarde locale du modèle et du scaler pour l'API d'inférence (CD)
        import joblib
